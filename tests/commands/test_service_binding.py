...
def test_assign_names_the_components_it_binds_to():
    """One call, `components` and all -- which is worth a note, because for a few hours on
    16 August this went through `add_services` on each component instead.

    The platform used to short-circuit the whole request the moment the service was already
    selected for the project: `success`, `components_updated: [frontend]`, and no binding. A
    practice run lost an authorization-wall to it and got a public URL answering 200 with no
    wall in front of it. Since configuring a service before binding it is the normal order,
    that was the common branch.

    It was fixed the same evening -- the endpoint now binds the components whether or not the
    service was already there, and says so in its description -- and measured against the
    sandbox before this went back to one call. The guard that outlives this comment is the
    step in playbook 01, which binds a service configured one step earlier and then reads
    `component list` instead of believing the answer."""
    route = respx.post(SERVICES).mock(return_value=_ok())
