...
**De lagen komen uit `zad service list -o json`, niet uit dit bestand.** Dat is geen
...
```sh
zad project describe --part components -o json | jq -e '[.components[].name] | sort == ["api","web"]'
```
...
```sh
zad service list -o json | jq -r '.[] | select(.configurable) | "\(.name)\t\(.targets|join(","))"'
```

**Controle:** de catalogus noemt zichzelf, en elke instelbare dienst heeft minstens één laag.

```sh
zad service list -o json | jq -e '
...
```sh
zad service config set keycloak --target project --generate-skeleton
```

### Diensten op projectniveau

```sh
zad service config set invite             --target project --set default-language=nl
zad service config set authorization-wall --target project --set banner='Alleen voor medewerkers'
zad service config set sleep-mode         --target project --set enabled=false
```
...
for SVC in invite authorization-wall sleep-mode; do
  zad service config get $SVC -o json | jq -e --arg s "$SVC" '
...
zad service config set metrics-scraper --component api
```

**Controle:** ze staan op het component dat genoemd is, en niet op het andere.

```sh
zad service config get health-check -o json | jq -e '
...
```sh
zad service config get publish-on-web -o json
```
...
zad service config get publish-on-web -o json | jq -e '[.configurations[]] | length == 2'

zad service config clear publish-on-web --target component --component api
zad service config get publish-on-web -o json | jq -e '
...
```sh
zad service config clear health-check --component web
zad service config get health-check -o json | jq -e '[.configurations[]] | length == 0'
```

## 7. Verborgen diensten

`zad service list` laat de interne varianten weg; `--all` toont ze. Ze horen niet in een
playbook thuis om aan te zetten, maar wel om te controleren dat de CLI ze niet verzint.

```sh
zad service list --all -o json | jq -e '[.[] | select(.hidden)] | length > 0'
zad service list -o json      | jq -e '[.[] | select(.hidden)] | length == 0'
```
